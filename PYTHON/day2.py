...
def score(dice):
    ...
    for x in range(1,6,4):
        ...
        if x==5:
            m=50
        if dice.count(x)>3:
            score+=(dice.count(x)-3)*m
        elif dice.count(x)<3:
            score+=dice.count(x)*m
...
ls=[int(random.uniform(1,5)),int(random.uniform(1,5)),
    int(random.uniform(1,5)),int(random.uniform(1,5)),int(random.uniform(1,5))]
print(ls)
print(score(ls))
